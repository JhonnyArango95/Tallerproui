/* ============================================
   GESTIÓN DE USUARIOS - COMPONENTE CON API REAL
   ============================================
...
import type { EmpleadoResponse } from '../../config/api';
...
interface User {
  ...
  role: string;         // Rol (ADMINISTRADOR, MECANICO, RECEPCION)
...
export function UserManagement() {
  /* ============================================
     ESTADO 1: LISTA DE USUARIOS (DESDE API)
     ============================================
  ...
  */
  ...
  /* ============================================
     EFECTO: CARGAR EMPLEADOS AL INICIO
     ============================================
  ...
  */
  ...
  /* ============================================
     FUNCIÓN: CARGAR EMPLEADOS DESDE API
     ============================================
  */
  const cargarEmpleados = async () => {
    try {
      ...
      const usuariosMapeados: User[] = empleados.map((emp: EmpleadoResponse) => ({
        id: emp.id,
  ...
  */
  const handleSubmit = async () => {
    ...
    if (!formData.firstName || !formData.lastName || !formData.email || !formData.phone || !formData.role || !formData.administrator) {
    ...
    // VALIDACIÓN: Para crear, la contraseña es obligatoria
    ...
    try {
      ...
      if (editingUserId) {
        /* ============================================
           ...
           ============================================ */
        await empleadosService.editarEmpleado(editingUserId, empleadoData);
        ...
        /* ============================================
           ...
           ============================================ */
        await empleadosService.crearEmpleado(empleadoData);
  ...
  */
  const handleDelete = async (userId: number) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este usuario?')) {
      try {
        await empleadosService.eliminarEmpleado(userId);
  ...
  */
  const filteredUsers = users.filter((user) => {
    ...
    const matchesRole = filterRole === 'todos' || user.role.toLowerCase() === filterRole.toLowerCase();
    const matchesStatus = filterStatus === 'todos' || user.status.toLowerCase() === filterStatus.toLowerCase();
                        ...
                        <span
                          className={`inline-flex items-center px-2 py-1 rounded text-xs ${
                            user.status.toUpperCase() === 'ACTIVO'
                              ...
                              : user.status.toUpperCase() === 'INACTIVO'
